...
test_that("assign_date() | `return` test", {
    ...
    expected <- list(bt_w = lubridate::as_datetime("1970-01-01 22:15:00"),
                     sprep_w = lubridate::as_datetime("1970-01-02 00:00:00"))
    expect_equal(object, expected)

    so_w <- hms::parse_hms("01:10:00")
    se_w <- hms::parse_hms("11:45:00")
    object <- assign_date(so_w, se_w, return = "list")
    expected <- list(so_w = lubridate::as_datetime("1970-01-01 01:10:00"),
                     se_w = lubridate::as_datetime("1970-01-01 11:45:00"))
    expect_equal(object, expected)
