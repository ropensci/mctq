...
data <- data %>%
  dplyr::mutate(dplyr::across(dplyr::matches("^id$|^wd$"), as.integer)) %>%
  dplyr::mutate(dplyr::across(
    dplyr::matches("^work$|^alarm_|^wake_|^reasons_f$"), as.logical)) %>%
  dplyr::mutate(dplyr::across(
    dplyr::matches("^bt_|^sprep_|^se_"), hms::parse_hm)) %>%
  dplyr::mutate(dplyr::across(
    dplyr::matches("^slat_|^si_"), ~ lubridate::dminutes(as.numeric(.x)))) %>%
  dplyr::mutate(dplyr::across(
    dplyr::matches("^le_"), ~ lubridate::as.duration(hms::parse_hm(.x))))
```
...
Please note that, although we tried to preserve the original authors naming pattern for the MCTQ functions, the name `sd` provokes a dangerous name collision with the widely used [stats::sd](https://rdrr.io/r/stats/sd.html) (standard deviation) function. That's why we named it as `sdu`. This is the only exception, all the other `mctq` functions maintain a strong naming resemblance with the original authors naming pattern.
...
Please note that by loading other packages the `ms()` function may be masked. This happens when you load the `lubridate` package (`library(lubridate)`) after loading `mctq` (`library(mctq)`). To assure you're using the `ms()` function from the `mctq` package, use the prefix `mctq::`, as shown below.

```{r, warnings = FALSE, message = FALSE}
data$msw <- mctq::ms(data$so_w, data$sd_w)
data$msf <- mctq::ms(data$so_f, data$sd_f)
