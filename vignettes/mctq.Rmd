...
This article takes a quick tour of all standard Munich Chronotype Questionnaire (MCTQ) main functions from the `mctq` package. Please see the function documentation and other articles/vignettes for more details.
...
Teaching you how to load your data in R is outside the scope of this article. For that, we recommend checking the [readr](https://readr.tidyverse.org/) package from [tidyverse](https://www.tidyverse.org/) (all packages from tidyverse are great!).

Our fictional MCTQ data will be generated with the code below. The naming of the variables follows the same naming scheme used in MCTQ articles. You can see the meaning of each variable by running `?std_mctq`.
...
Seems too complicated? Don't worry! You don't have to know how to handle [lubridate](https://lubridate.tidyverse.org/) and [hms](https://hms.tidyverse.org/) to do this. We developed a special function called `convert()` to help you.

`convert()` allows you to convert and parse data to different kinds of R objects. Here we are interested in two of them: `Duration` objects, to store durations (like sleep latency), and `hms` objects, to store local time values (like bedtime).
...
As you can see, our data came in different formats. For example, the column `bt_w` (local time of going to bed on workdays) is in hours:minutes format, while `slat_w` (sleep latency on workdays) is a duration in minutes.
...
`fd()` is a simple function that allows you to compute the difference between the total of days in a week and the number of workdays per week (`wd`). It takes only `wd` as argument.
...
The output must be the sum of `sprep` and `slat` considering the circularity of time.
...
The output must be the sum of `se` and `si` considering the circularity of time.
...
The output must be the difference between `se` and `so` considering the circularity of time.
...
The output must be the difference between `gu` and `bt` considering the circularity of time.
...
The output must be the sum of `so` with the half of `sd` duration considering the circularity of time.
...
The output must be the weighted mean of `sd_w` and `sd_f`, with `wd` and `fd(wd)` as weights, considering the circularity of time.
...
The output must be the weighted mean of `le_w` and `le_f`, with `wd` and `fd(wd)` as weights, considering the circularity of time.
...
The output must be the difference between `msf` and `msw` considering the circularity of time.
...
If you're using [RStudio](https://rstudio.com/), you can run all the code showed above and then type `View(data)` in the console to explore the end result.
