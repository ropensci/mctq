...
This article shows a possible work around to a common problem encountered when dealing with a __standard__ Munich Chronotype Questionnaire (MCTQ) and __$\mu$MCTQ__ data.
...
To illustrate this possible different approach, consider the following.

If a subject __do not have a regular work schedule__ (`wd == 0`), __only answered the work-free days section__, and __do not use an alarm clock on his/her free days___ (_i.e_ `alarm_f == FALSE`), it would be fair to assume that there's no sleep correction (`sc`) to be made, therefore, his/her chronotype (`msf_sc`) must be equal to his/her midsleep on work-free days (`msf`).

Following this same line of thought, we can also say that:

* `sd_week` (average weekly sleep duration) value must be equal to `sd_f` (sleep duration on work-free days), since the subject do not have workdays.
* `sloss_week` (weekly sleep loss) value must be equal to `0s`, since there's no sleep debt.
* `le_week` (average weekly light exposure) value must be equal to `le_f` (light exposure on work-free days), since there's no workdays.
...
The opposite scenario, _i.e._ when the subject __works 7 days per week__ (`wd == 7`) and __only answered the workdays section__, can also have different outputs. `msf_sc` (chronotype), `sloss_week`, `sjl_rel`, `sjl` should still produce a `NA` as output, since there are no way to know the real behavior of the subject sleep-wake cycle. But `sd_week` and `le_week` can have different values. Accordingly to this reasoning, that must be as the following.

* `sd_week` (average weekly sleep duration) value must be equal to `sd_w` (sleep duration on workdays), since the subject do not have work-free days.
* `le_week` (average weekly light exposure) value must be equal to `le_w` (light exposure on work-free days), since the subject do not have work-free days.

If you agree to this line of thought, we recommend creating dummy variables to identify those two situations and them changing the case values as mentioned. You can see this procedure in action with the [data wrangling algorithms](https://github.com/gipsousp/mctq/blob/master/data-raw/std_mctq.R#L923) made to produce the fictional `std_mctq` dataset, provided by the `mctq` package.

Please note that this work around is not mentioned or endorsed by the original MCTQ authors. If you use it, you must mention this reasoning in you methods section.
